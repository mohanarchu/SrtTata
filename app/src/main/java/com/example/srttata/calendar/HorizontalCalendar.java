...
import com.example.srttata.calendar.adapter.DaysAdapter;
import com.example.srttata.calendar.adapter.HorizontalCalendarBaseAdapter;
import com.example.srttata.calendar.adapter.MonthsAdapter;
import com.example.srttata.calendar.model.CalendarItemStyle;
import com.example.srttata.calendar.model.HorizontalCalendarConfig;
...
import com.example.srttata.calendar.utils.HorizontalSnapHelper;
...
public final class HorizontalCalendar {
     ...
     */
    private HorizontalCalendar(Builder builder, HorizontalCalendarConfig config, CalendarItemStyle defaultStyle, CalendarItemStyle selectedItemStyle) {
    ...
    /* Init Calendar View */
    private void init(View rootView, final Calendar defaultSelectedDate, HorizontalCalendarPredicate disablePredicate,
    ...
    public void moveToNextMonth(Calendar date, boolean immediate) {
        int datePosition = positionOfDate(date);
        if (immediate) {
            centerToPositionWithNoAnimation(datePosition);
            if (calendarListener != null) {
    ...
    public void centerCalendarToPosition(final int position) {
        ...
        if (position != -1) {
            int relativeCenterPosition = Utils.calculateRelativeCenterPosition(position,
                    calendarView.getPositionOfCenterItem(), numberOfDatesOnScreen / 2);
     ...
     */
    private int positionOfDate(Calendar date) {
